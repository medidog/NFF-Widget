...
/**
 * Converts a JSON or Array defining string to the corresponding JavaScript object.
 * When secure paramteter is true, it uses the JSON library which can be found at http://json.org.
 * Unless the secure parameter is used it is only "a-bit-secure". Use with caution.
 * 
 * @param {Boolean} secure Set true to turn on extreme secureity. Needs JSON library.
 */
String.prototype.toObject=function(secure)
{
	if ((this.charCodeAt(this.length-1)-this.charCodeAt(0))!=2 || (this.charAt(0)!='[' && this.charAt(0)!='{')) //do NOT user RegExp here since it crashes with very large strings
		return false;
